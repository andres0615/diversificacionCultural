...
namespace mvc\model\table {
  ...
  class tableBaseClass implements tableInterface {
     ...
     * @throws \PDOException
     */
    public static function getAll($table, $fields, $deletedLogical = true, $orderBy = null, $order = null, $limit = null, $offset = null, $where = null, $like = null, $join = null) {
      try {
        ...
        if (is_array($join)) {
          ...
          foreach ($join as $joinTable => $joinAtributes) {
            $sql = $sql . ' ' . $joinAtributes['JOIN_TYPE'] . ' JOIN ' . $joinAtributes['TABLE2_NAME'] . ' ' . $joinAtributes['TABLE2_ALIAS'] . ' ON ' . $joinAtributes['TABLE2_ALIAS'] . '.' . $joinAtributes['TABLE2_COLUMN'] . ' = ' . $joinAtributes['TABLE1_NAME'] . '.' . $joinAtributes['TABLE1_COLUMN'] . ' ';
          }
        }
          ...
          foreach ($like as $field => $value) {
            if ($flag === false) {
              if (is_numeric($field)) {
                ...
                $sql = $sql . ' WHERE ' . $field . ' LIKE ' . ((is_numeric($value)) ? $value : "'$value%'") . ' ';
              }
              $flag = true;
            } else {
              if (is_numeric($field)) {
                ...
                $sql = $sql . ' AND ' . $field . ' LIKE ' . ((is_numeric($value)) ? $value : "'$value%'") . ' ';
              }
            }
          }
        }
        ...
        if (config::getShowSqlGetAll() === true) {
