$(document).ready(function(){
  ...
  $.fn.showStickySuccessToast = function(mesage) {
  ...
  $.fn.showStickyErrorToast = function(mesage) {
    $('#mensaje').toastmessage('showToast', {
      text     : mesage,
      sticky   : true,
      position : '',
  ...
  $.fn.signupRedirection = function(url) {
    setTimeout(function(){
      window.location = url;
    }, 2000);
  }
  ...
  $('#submit-user-form').click(function(){
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (flag === "true") {
          $(document).showStickySuccessToast(mesage).signupRedirection(myarr[2]);
        } else {
          $(document).showStickyErrorToast(mesage);
        }
      }
    });
  });
