...
//      sleep(1);
      ...
      if (request::getInstance()->hasPost('borrarFiltros')) {
        ...
        if (request::getInstance()->getPost('borrarFiltros') === 'true') {
          session::getInstance()->deleteAttribute('eventosUiFilter');
          session::getInstance()->deleteAttribute('eventosUiWhere');
        }
      }
      ...
      if (request::getInstance()->hasPost('filter')) {
        ...
        if (strlen(trim($filter['eventoUsuario'])) > 0) {
          $where[eventoTableClass::USUARIO_ID] = $filter['eventoUsuario'];
        }

        if (strlen(trim($filter['eventoCategoria'])) > 0) {
          $where[eventoTableClass::CATEGORIA_ID] = $filter['eventoCategoria'];
        }

        $this->filter = request::getInstance()->getPost('filter');

        session::getInstance()->setAttribute('eventosUiFilter', $this->filter);
        session::getInstance()->setAttribute('eventosUiWhere', $where);

      } else if (session::getInstance()->hasAttribute('eventosUiWhere')) {
        $where = session::getInstance()->getAttribute('eventosUiWhere');
        $this->filter = session::getInstance()->getAttribute('eventosUiFilter');
      }
      ...
      view::includePartial('eventos/registrosTemplate.html', 
              array(
