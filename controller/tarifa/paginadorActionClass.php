...
//      sleep(1);  
      ...
      if (request::getInstance()->hasPost('borrarFiltros')) {
        ...
        if (request::getInstance()->getPost('borrarFiltros') === 'true') {
          session::getInstance()->deleteAttribute('tarifaFilter');
          session::getInstance()->deleteAttribute('tarifaWhere');
        }
      }
      ...
      if (request::getInstance()->hasPost('filter')) {    
        ...
        if (strlen(trim($filter['tarifaNombre'])) > 0) {
          $where[tarifaTableClass::DESCRIPCION] = $filter['tarifaNombre'];          
        }
        
        if (strlen(trim($filter['tarifaFechaInicial'])) > 0 && strlen(trim($filter['tarifaFechaFinal'])) > 0) {
          $where[tarifaTableClass::CREATED_AT] = array(
              date(config::getFormatTimestamp(), strtotime($filter['tarifaFechaInicial'] . ' 00:00:00' )),
              date(config::getFormatTimestamp(), strtotime($filter['tarifaFechaFinal'] . ' 23:59:59' ))
          );
        }
        
        $this->filter = request::getInstance()->getPost('filter');        
        
        session::getInstance()->setAttribute('tarifaFilter', $this->filter);
        session::getInstance()->setAttribute('tarifaWhere', $where);
        
      } else if (session::getInstance()->hasAttribute('tarifaWhere')) {
          $where = session::getInstance()->getAttribute('tarifaWhere');
          $this->filter = session::getInstance()->getAttribute('tarifaFilter');          
      }
      ...
      if (session::getInstance()->hasAttribute('tarifaWhere')) {
        $filter = session::getInstance()->getAttribute('tarifaFilter');
        $where = session::getInstance()->getAttribute('tarifaWhere');
      } 
      ...
      $this->visibleRows = count($this->objTarifas);
      ...
      $this->totalRows = count(tarifaTableClass::getAll($fields));  
      

      view::includePartial('tarifa/registrosTemplate.html', 
              array(
                  'objTarifas' => $this->objTarifas,
                  'page' => $this->page,
                  'pages' => $this->pages,
